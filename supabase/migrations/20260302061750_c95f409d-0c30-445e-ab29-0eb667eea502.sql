...
-- Sprint 12F: Fix RPC ledger direction mapping (IN/OUT → CREDIT/DEBIT)
-- Also add cash_account_id + account_snapshot for ledger entries
...
BEGIN
  ...
  -- Map cashflow direction to ledger direction
  v_ledger_direction := CASE WHEN p_direction = 'OUT' THEN 'DEBIT' ELSE 'CREDIT' END;
  ...
  -- For settlement payments: create cash_out first
  ...
  -- 2. Ledger entry (DEBIT/CREDIT, with account snapshot)
  INSERT INTO public.ledger_entries (
    ...
    COALESCE(v_account_snapshot, '{}'::JSONB)
  )
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'ledger_direction', v_ledger_direction,
      'amount', p_amount,
