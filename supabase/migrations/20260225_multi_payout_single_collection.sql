...
-- Migration: 20260225_multi_payout_single_collection.sql
--
-- PROBLEM: Khi ghi nhận tiền OTA về cho nhiều payout cùng lúc,
-- hệ thống tạo N phiếu thu riêng biệt. Yêu cầu: 1 phiếu thu duy nhất.
--
-- SOLUTION: Junction table collection_payout_allocations
-- 1 hotel_collects → N collection_payout_allocations → N ota_payouts
-- ============================================
...
Used when multiple OTA payouts share a single bank transfer / transaction.';
...
-- ============================================
-- PART B: BATCH RPC - CREATE MULTI PAYOUT CASH-IN
-- Creates 1 hotel_collects + N allocations + 1 ledger + 1 cashflow
-- ============================================
...
BEGIN
  ...
  -- Validate sum of allocations = total
  ...
  -- ========================================
  -- 5. VALIDATE ALL PAYOUTS EXIST & GET OTA SOURCE
  -- ========================================
  ...
  -- ========================================
  -- 6. INSERT SINGLE HOTEL_COLLECTS (1 phiếu thu)
  -- ========================================
  ...
  -- ========================================
  -- 7. INSERT ALLOCATION RECORDS (junction table)
  -- ========================================
  FOR v_i IN 1..array_length(p_payout_ids, 1) LOOP
  ...
  -- ========================================
  -- 8. INSERT LEDGER ENTRY (1 entry, total amount)
  -- ========================================
  ...
  -- ========================================
  -- 9. INSERT CASHFLOW ENTRY (1 entry, total amount)
  -- ========================================
...
-- ============================================
-- PART C: RECALCULATE STATUS (junction table only)
-- ============================================
...
-- ============================================
-- PART D: UPDATED REVERSE (handles junction table)
-- ============================================
...
BEGIN
  ...
  -- 6. RECALCULATE STATUS FOR ALL LINKED PAYOUTS (via junction table)
  FOR v_alloc IN 
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'source_payout_id', v_collection.source_payout_id
    ),
...
-- ============================================
-- PART E: Also update the old single-payout RPC to use new status fn
-- ============================================
...
BEGIN
  ...
  -- Also insert into junction table for consistency
  INSERT INTO collection_payout_allocations (collection_id, payout_id, allocated_amount)
...
-- reverse_ota_payout_cashin already granted in previous migration
...
Input: arrays of payout_ids + amounts + shared transaction details.
Output: collection_id (UUID) of the single hotel_collects record.';
