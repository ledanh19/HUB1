-- ============================================================
-- VOID SETTLEMENT – Additive Migration
-- ============================================================
-- Date: 2026-03-01
-- Purpose: Add VOID status to host_settlements + atomic RPC
-- NON-BREAKING: Only adds new constraint values, columns, function
-- ============================================================

-- ============================================================
-- PART 1: SCHEMA CHANGES (ADDITIVE ONLY)
-- ============================================================

-- 1.1 Expand status constraint to include 'VOID'
...
-- 1.2 Add void metadata columns (additive, nullable)
ALTER TABLE public.host_settlements 
...
-- idx_payment_requests_settlement_id already exists (from 20251216050318)
-- idx_cash_outs_payment_request_id already exists (from 20251216050318)

-- ============================================================
-- PART 2: RPC – void_settlement_secure
-- ============================================================
-- Follows the exact pattern of finalize_settlement_secure / close_settlement_secure
-- in 20260106_permission_system_v21_unified.sql
...
BEGIN
  ...
  -- Lock settlement row (FOR UPDATE prevents concurrent void)
  ...
  -- IDEMPOTENCY: If already VOID, return immediately (no duplicate audit)
  IF v_settlement.status = 'VOID' THEN
    RETURN p_settlement_id;
  END IF;

  -- Guard: only SETTLED can be voided
  ...
  -- Guard: total_paid_amount must be 0 (snapshot check)
  ...
  -- Guard: no cash_outs via payment_requests
  ...
  -- Guard: DEFENSE-IN-DEPTH – check cashflow_entries (true SOT for payments)
  -- total_paid_amount is a snapshot that may drift from actual cashflow records.
  -- This guard catches the edge case where total_paid_amount=0 but real payments exist.
  IF EXISTS (
  ...
  -- Guard: period lock (check settlement's finalized date)
  ...
  -- ============================================
  -- STEP 2: UPDATE host_settlements → VOID
  -- ============================================
  ...
  -- ============================================
  -- STEP 3: CASCADE – Cancel linked payment_requests
  -- ============================================
  -- Set-based audit: snapshot before update
  INSERT INTO audit_logs (action, entity, entity_id, user_id, before_data, after_data)
  SELECT 
  ...
  -- ============================================
  -- STEP 4: CASCADE – Unlock host_supply_segments
  -- ============================================
  -- Set-based audit
  INSERT INTO audit_logs (action, entity, entity_id, user_id, before_data, after_data)
  SELECT 
  ...
  -- ============================================
  -- STEP 5: CASCADE – Unlock host_extra_charges
  -- ============================================
  INSERT INTO audit_logs (action, entity, entity_id, user_id, before_data, after_data)
  SELECT 
  ...
  -- ============================================
  -- STEP 6: CASCADE – Unlock host_surcharges
  -- ============================================
  INSERT INTO audit_logs (action, entity, entity_id, user_id, before_data, after_data)
  SELECT 
