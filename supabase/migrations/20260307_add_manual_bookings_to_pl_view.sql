-- Migration: Add manual_bookings to analytics_historical_daily_pl_v
-- This aligns the Dashboard analytics view with P&L SOT (usePLCalculator)
-- which now includes both bookings_mirror AND manual_bookings.
...
-- OTA bookings from bookings_mirror (scoped to An Gia properties)
ota_bookings AS (
  SELECT
...
-- Manual bookings (no channex_property_id, no property scope needed)
manual_booking_rows AS (
  SELECT
    ...
    NULL::uuid AS channex_property_id
