...
-- Migration: 20260104_ota_payout_ledger_manual.sql
-- 
-- QUYẾT ĐỊNH NGHIỆP VỤ:
-- ❌ KHÔNG dùng account_mapping_rules cho OTA Payout
-- ❌ KHÔNG auto-resolve
-- ❌ KHÔNG fallback default
-- ✅ MANUAL selection: Kế toán PHẢI chọn tay cash_account_id
-- ============================================

-- ============================================
-- PART A: RPC - CREATE OTA PAYOUT CASH-IN ATOMIC
-- Ghi nhận tiền OTA về với ledger entry
-- ============================================

CREATE OR REPLACE FUNCTION public.create_ota_payout_cashin_atomic(
  p_payout_id UUID,
  p_amount NUMERIC,
  p_cash_account_id UUID,           -- REQUIRED: Manual selection
  p_received_at DATE,
...
BEGIN
  ...
  -- ========================================
  -- 1. PERMISSION CHECK (admin/ke_toan only)
  -- ========================================
  SELECT EXISTS (
  ...
  -- ========================================
  -- 3. VALIDATE CASH ACCOUNT (REQUIRED - NO FALLBACK)
  -- ========================================
  ...
  -- ========================================
  -- 4. CHECK ACCOUNTING PERIOD LOCK
  -- ========================================
  ...
  -- ========================================
  -- 6. INSERT HOTEL_COLLECTS (Source of Truth)
  -- ========================================
  ...
  VALUES (
    'OTA-PAYOUT-' || LEFT(p_payout_id::TEXT, 8),
    p_amount,
    p_payment_method,
    'ROOMRISE',
    'OTA',
    ...
    'COLLECT',
    p_bank_reference,
    COALESCE(p_note, 'Tiền OTA ' || v_payout.ota_source || ' về qua ' || 
      CASE WHEN p_payment_method = 'BANK_TRANSFER' THEN 'chuyển khoản' ELSE COALESCE(p_payment_channel, 'UPC') END),
    'COLLECTED'
  )
  RETURNING id INTO v_collection_id;
  
  -- ========================================
  -- 7. INSERT LEDGER ENTRY (DEBIT - tiền vào)
  -- 🔒 KHÔNG gọi resolve_account_mapping()
  -- 🔒 Dùng account từ user selection
  -- ========================================
  INSERT INTO ledger_entries (
    ...
    entry_type,
    cash_account_id,
    account_snapshot,       -- Snapshot bất biến
    direction,
    amount,
    currency,
    counterparty_type,
  ...
  VALUES (
    ...
    p_received_at,
    now(),
    'OTA_PAYOUT',           -- source_type
    v_collection_id,        -- Link to hotel_collects, NOT ota_payouts
    'ORIGINAL',
    p_cash_account_id,      -- Manual selection từ UI
    jsonb_build_object(     -- Snapshot bất biến
      ...
      'selected_at', now()::TEXT,
      'selected_by', v_user_id::TEXT
    ),
    'DEBIT',                -- Tiền vào = DEBIT
    p_amount,
    'VND',
    'OTA',
    v_payout.ota_source,
    v_payout.ota_source || ' Payout',
    v_user_id,
  ...
  -- ========================================
  -- 8. INSERT CASHFLOW ENTRY (maintain legacy flow)
  -- ========================================
  ...
  VALUES (
    ...
    'OTA',
    'OTA ' || v_payout.ota_source || ' payout - ' || COALESCE(p_bank_reference, ''),
    v_user_id
  );
  
  -- ========================================
  -- 9. UPDATE OTA PAYOUT STATUS (derived from collections)
  -- ========================================
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'cash_account_name', v_account.account_name,
      'cash_account_bank', v_account.bank_name,
...
-- Đảo bút toán OTA Payout (sửa sai)
-- ============================================
...
BEGIN
  ...
  -- ========================================
  -- 3. CHECK ACCOUNTING PERIOD LOCK
  -- ========================================
  ...
  -- ========================================
  -- 4. FIND AND REVERSE LEDGER ENTRY
  -- ========================================
  ...
  IF v_ledger_entry IS NOT NULL THEN
    -- Use existing reverse_ledger_entry function
    SELECT reverse_ledger_entry(v_ledger_entry.id, p_reason) INTO v_reversal_id;
  END IF;
  
  -- ========================================
  -- 5. VOID THE COLLECTION (soft delete)
  -- ========================================
  INSERT INTO hotel_collects (
    unified_booking_id,
    amount_collected,
  ...
  VALUES (
    ...
    v_collection.amount_collected,
    v_collection.payment_method,
    v_collection.payee_type,
    v_collection.payer_type,
    v_collection.related_type,
    v_collection.source_payout_id,
    p_collection_id,  -- Reference to original
    now(),
    v_user_id,
    'VOID',
    'VOID: ' || p_reason,
...
CREATE OR REPLACE FUNCTION public.recalculate_ota_payout_status(
  p_payout_id UUID
) RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE hc.source_payout_id = p_payout_id
    AND hc.related_type = 'OTA_PAYOUT'
...
COMMENT ON FUNCTION public.create_ota_payout_cashin_atomic IS 
'Ghi nhận tiền OTA Payout về với ledger entry.
REQUIRED: p_cash_account_id - Kế toán PHẢI chọn tay tài khoản nhận tiền.
KHÔNG dùng resolve_account_mapping().
KHÔNG auto-resolve hoặc fallback default.';

COMMENT ON FUNCTION public.reverse_ota_payout_cashin IS
'Đảo bút toán OTA Payout cash-in.
Cách duy nhất để sửa sai: Reverse và tạo mới.
KHÔNG cho edit trực tiếp ledger entry.';
