...
-- Allows multiple items per type per payout (e.g. multiple DISPUTE rows)
...
-- Idempotency unique: (payout_id, idempotency_key) where key is not null
...
-- ─── PHASE 2A: UPDATE create_ota_payout_reconciliation_item_atomic ───
-- Remove ON CONFLICT upsert, use plain INSERT + optional idempotency_key
...
BEGIN
    ...
    -- Skip bank fees (handled by existing RPC)
    IF v_item.item_type = 'BANK_TRANSFER_FEE' THEN
        RETURN jsonb_build_object(
...
BEGIN
    ...
    LOOP
        IF p_dry_run THEN
            -- Classify only (no persist in dry run)
...
-- ─── BACKFILL: Classify existing items (safe, only sets adj_category) ───
DO $$
DECLARE
    v_item RECORD;
BEGIN
