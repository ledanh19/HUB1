...
BEGIN
  ...
  FOR v_i IN 1..array_length(p_payout_ids, 1) LOOP
    ...
    -- EXPLICIT GUARD: ensure received_at is always set from user's chosen date
    -- recalculate_ota_payout_status_v2 only sets it when status=RECEIVED,
    -- but we want it set regardless (for PARTIAL too)
    UPDATE ota_payouts
