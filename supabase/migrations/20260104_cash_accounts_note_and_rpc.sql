...
-- Migration: 20260104_cash_accounts_note_and_rpc.sql
-- 
-- MỤC TIÊU:
-- 1. Thêm column note cho cash_accounts
-- 2. RPC create_cash_account_atomic tự sinh account_code
-- 3. RPC update_cash_account_atomic với audit
-- 4. UI không cần nhập mã - backend tự tạo
-- 
...
BEGIN
  ...
  -- Common Vietnamese banks mapping
 ...
 * 
 * Tạo tài khoản tiền mới với:
 * - Auto-generate account_code (user không cần nhập)
 * - Handle collision nếu code đã tồn tại
 * - Set default + unset các account khác
 * - Audit log
 * 
 * ACCOUNT CODE FORMAT:
 * - BANK: BANK_<short>_<last4>  (VD: BANK_VCB_6789)
 * - CASH: CASH_<seq>            (VD: CASH_001)
 * - DIGITAL_WALLET: WALLET_<provider>_<last4_or_alias> (VD: WALLET_MOMO_1234)
 */
...
BEGIN
  ...
  CASE p_account_type
    ...
    WHEN 'DIGITAL_WALLET' THEN
      -- Use bank_name as provider for wallet (MOMO, ZALOPAY, etc.)
  ...
  -- ========================================
  -- 5. HANDLE COLLISION - ADD SUFFIX IF NEEDED
  -- ========================================
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'account_number', CASE 
        WHEN p_account_number IS NOT NULL THEN '****' || v_last4 
        ELSE NULL 
      END,
      'note', p_note,
...
COMMENT ON FUNCTION public.create_cash_account_atomic IS
'Tạo tài khoản tiền mới với auto-generated account_code.
UI không cần nhập mã - backend tự sinh.
Audit log tự động ghi nhận.';
...
BEGIN
  ...
  -- ========================================
  -- 3. UPDATE ACCOUNT (KEEP account_code)
  -- ========================================
  UPDATE cash_accounts
  SET 
  ...
  -- ========================================
  v_last4 := CASE 
    WHEN p_account_number IS NOT NULL AND LENGTH(p_account_number) >= 4 
    THEN '****' || RIGHT(p_account_number, 4)
    ELSE NULL 
  END;
  ...
  VALUES (
    ...
    v_user_id,
    jsonb_build_object(
      'account_name', v_old_record.account_name,
      'account_type', v_old_record.account_type,
      'bank_name', v_old_record.bank_name,
      'note', v_old_record.note
    ),
    jsonb_build_object(
      'account_name', p_account_name,
      'account_type', p_account_type,
