...
-- Created: 2026-03-02
-- 
-- Strategy: Add V1 content columns to V2 tables, backfill data,
-- re-point FKs, drop overlay views + V1 mirror tables.
-- 
-- SAFE: All additive first, destructive at the end.
-- ============================================================
...
-- sent_at may already exist — add if not
ALTER TABLE public.email_messages
...
-- Only update rows that have matching V1 data
...
-- Need to resolve V1 mirror thread_id → V2 thread_id via provider_thread_id
...
SELECT
  ...
  m1.date,
  -- Extract sender from from_json for V2 sender column
  CASE
...
-- Map V1 mirror thread_id → V2 thread_id
DO $$
DECLARE
  _has_fk boolean;
BEGIN
  -- Check if FK exists pointing to mirror
  SELECT EXISTS(
  ...
  IF _has_fk THEN
    ...
    RAISE NOTICE 'Re-pointed email_thread_workflow FK to V2';
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Re-pointed email_thread_notes FK to V2';
END $$;
...
-- CASCADE drops remaining FKs from mirror tables
...
-- authenticated can read new content columns
