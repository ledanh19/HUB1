...
import { useApplyDeposit, HostDeposit } from "@/hooks/useHostDeposits";
...
export function ApplyDepositDialog({
  ...
  const applyAmount = Math.min(deposit.deposit_amount, payableRemaining);
  const exceedsPayable = deposit.deposit_amount > payableRemaining;
  const partnerMismatch = bookingPartnerId && deposit.partner_id !== bookingPartnerId;
  ...
  // GUARDRAIL: Block if deposit not yet PAID (chưa chi tiền)
  const blockedNotPaid = deposit.approval_status !== "PAID";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Cấn trừ Deposit</DialogTitle>
          <DialogDescription>
            Xác nhận cấn trừ deposit vào công nợ Host
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {blockedNotPaid && (
            <Alert className="bg-warning/10 border-warning/20">
              ...
              <AlertDescription className="text-warning">
                <strong>Chặn thao tác:</strong> Deposit này chưa được xác nhận chi tiền (trạng thái: {
                  deposit.approval_status === "PENDING" ? "Chờ duyệt" :
                    deposit.approval_status === "APPROVED" ? "Đã duyệt - chờ chi" :
                      deposit.approval_status === "REJECTED" ? "Đã từ chối" : deposit.approval_status
                }). Chỉ có thể cấn trừ deposit đã chi tiền.
              </AlertDescription>
            </Alert>
          )}
          ...
          {!blockedNotPaid && blockedNotCheckedOut && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                <strong>Chặn thao tác:</strong> Không thể cấn trừ deposit trước khi booking đã trả phòng.
          ...
          {!blockedNotPaid && partnerMismatch && (
            <Alert variant="destructive">
              <XCircle className="h-4 w-4" />
              <AlertDescription>
                <strong>Chặn thao tác:</strong> Deposit này thuộc Host khác, không khớp với booking hiện tại.
              </AlertDescription>
            </Alert>
          )}
          ...
          {exceedsPayable && !blockedNoPayable && !blockedNotPaid && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Deposit ({formatCurrency(deposit.deposit_amount)}) lớn hơn công nợ còn lại.
          ...
          <p className="text-xs text-muted-foreground">
            Lưu ý: Thao tác cấn trừ không tạo cashflow mới. Deposit đã được ghi nhận cash-out khi xác nhận chi tiền.
          </p>
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            {canApply && (
              <Button onClick={handleApply} disabled={applyDeposit.isPending}>
                {applyDeposit.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
                Xác nhận cấn trừ
              </Button>
            )}
          </div>
