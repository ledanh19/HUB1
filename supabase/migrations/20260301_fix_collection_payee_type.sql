-- FIX: Allow passing payee_type to create_collection_ledger_atomic
-- Previously hardcoded to 'ROOMRISE', causing Host thu hộ = 0 in settlement
-- Now accepts p_payee_type with DEFAULT 'ROOMRISE' for backward compatibility
...
BEGIN
  ...
  -- Only create cashflow + ledger for ROOMRISE payee (Host collections don't go through Roomrise books)
