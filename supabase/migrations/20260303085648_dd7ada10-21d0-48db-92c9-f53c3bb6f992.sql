...
-- Fix confirm_room_rounding_secure: collection_type 'ROOM' → 'COLLECT'
-- to comply with refund_void_requires_related constraint
...
BEGIN
  ...
  -- GUARD 4: Compute actual room remaining
  ...
  IF v_room_remaining <= 0 THEN
    RAISE EXCEPTION 'ALREADY_PAID: remaining=% — booking is already fully paid',
      v_room_remaining;
  END IF;

  IF v_room_remaining > v_tolerance THEN
    RAISE EXCEPTION 'REMAINING_OVER_TOLERANCE: remaining=% exceeds tolerance=%',
      v_room_remaining, v_tolerance;
  END IF;

  -- ACTION: Insert hotel_collects — FIX: collection_type = 'COLLECT' (not 'ROOM')
  INSERT INTO hotel_collects (
    ...
    'SYSTEM',
    'ROOM',
    'COLLECT',            -- FIX: was 'ROOM', must be 'COLLECT' per constraint
    'ROUNDING_WRITE_OFF',
    'COLLECTED',
    now(),
    v_user_id
  ...
  INSERT INTO audit_logs (
    event_time, user_id, action, entity, entity_id, after_data
  ) VALUES (
    now(), v_user_id, 'BOOKING_ROUNDING_APPLIED', 'unified_bookings', p_booking_id,
    jsonb_build_object(
...
COMMENT ON FUNCTION public.confirm_room_rounding_secure IS
  ...
  'FIX: uses collection_type=COLLECT (not ROOM) to comply with refund_void_requires_related constraint. '
  'Guards: amount ≤ 1000 VND, booking exists, idempotent, computed remaining matches.';
