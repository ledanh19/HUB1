...
-- ============================================================================
-- Patch: Update cancel_payment_request_secure to support APPROVED status
-- Also adds cash_out guard for APPROVED requests
-- ============================================================================
...
BEGIN
  ...
  -- For APPROVED: guard against existing cash_outs or hotel_collects
  IF v_request.status = 'APPROVED' THEN
    ...
    SELECT EXISTS(
      SELECT 1 FROM hotel_collects WHERE payment_request_id = p_request_id
...
COMMENT ON FUNCTION public.cancel_payment_request_secure(UUID, TEXT) IS
  'Cancel PENDING or APPROVED payment request. APPROVED requires no existing cash_outs/hotel_collects. '
  'Auth + RBAC guarded. FOR UPDATE row lock. Idempotent. Full audit trail.';
