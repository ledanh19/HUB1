...
-- Run this migration to ensure all collections have ledger entries
-- ================================================================

-- Create RPC function for on-demand backfill
...
BEGIN
  ...
  IF v_default_account_id IS NULL THEN
    -- Try to get any non-archived account
    SELECT id INTO v_default_account_id
    FROM cash_accounts
...
COMMENT ON FUNCTION public.backfill_missing_ledger_entries IS
  'Backfill ledger entries for hotel_collects that do not have one.
   Returns the number of entries created.
   Safe to run multiple times - only creates entries for collections without ledger entries.';
