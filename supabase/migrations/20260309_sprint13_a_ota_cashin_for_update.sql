-- ============================================================================
-- SPRINT 13.A — OTA Cash-In FOR UPDATE (Race Condition Fix)
-- ============================================================================
-- Date: 2026-03-09
-- Risk: M1 — Concurrent cash-in requests can over-allocate payout amounts
-- Fix:  Add FOR UPDATE row lock on ota_payouts before reading remaining amount
--
-- NON-BREAKING. Signature unchanged. Only adds row lock to prevent race.
-- ============================================================================
...
DECLARE
  ...
  -- Sprint 13.A: remaining amount tracking
...
BEGIN
  ...
  FOR v_i IN 1..array_length(p_payout_ids, 1) LOOP
    ...
    -- ════════════════════════════════════════════════════════════════
    -- Sprint 13.A FIX: FOR UPDATE to prevent concurrent over-allocation
    -- Lock order: ota_payouts → allocations → ledger/cashflow
    -- ════════════════════════════════════════════════════════════════
    ...
    -- Sprint 13.A: Verify remaining amount AFTER lock to prevent over-allocation
    SELECT COALESCE(SUM(cpa.allocated_amount), 0)
    INTO v_total_allocated
    FROM collection_payout_allocations cpa
    WHERE cpa.payout_id = v_payout_id;

    v_remaining := COALESCE(v_payout.net_payout_amount, v_payout.total_amount, 0) - v_total_allocated;

    -- Sprint 13.A: Guard against dirty data (remaining already negative)
    IF v_remaining < 0 THEN
      RAISE EXCEPTION 'NEGATIVE_REMAINING_STATE: payout_id=% remaining=% — '
        'data inconsistency detected, cannot proceed with cash-in. '
        'Investigate existing allocations before retrying.',
        v_payout_id, v_remaining;
    END IF;

    IF v_amount > v_remaining THEN
      RAISE EXCEPTION 'OVER_ALLOCATION: Payout % remaining=%, requested=%',
        v_payout_id, v_remaining, v_amount;
    END IF;
    ...
    -- 4f. Audit log (enhanced with remaining amount tracking)
    ...
    VALUES ('OTA_PAYOUT_CASHIN_ATOMIC', 'hotel_collects', v_collection_id::TEXT, v_user_id,
      jsonb_build_object(
        ...
        -- Sprint 13.A: remaining tracking
        'remaining_before_allocation', v_remaining,
        'total_allocated_before', v_total_allocated
      ));
...
COMMENT ON FUNCTION public.create_multi_payout_cashin_atomic IS
  'Sprint 13.A: Atomic OTA payout cash-in with FOR UPDATE row lock to prevent '
  'concurrent over-allocation. Creates hotel_collects, allocations, ledger, cashflow, '
  'recalculates status, and posts bank fee + adjustment ledger entries — all in ONE '
  'transaction. Concurrent requests serialize on payout row lock.';
