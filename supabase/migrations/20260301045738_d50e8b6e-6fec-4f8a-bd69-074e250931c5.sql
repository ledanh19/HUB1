-- Drop old 10-param overload to avoid ambiguity
DROP FUNCTION IF EXISTS public.create_collection_ledger_atomic(text, numeric, text, text, text, text, uuid, text, uuid, text);

-- Recreate with 11 params including p_payee_type
...
BEGIN
  ...
  IF is_period_locked(v_org_id, v_entry_date) THEN
    RAISE EXCEPTION 'Kỳ kế toán đã khóa cho ngày %', v_entry_date;
  END IF;
