...
-- =============================================
-- PROMPT 1: TẠO unified_payments VIEW
-- Gom hotel_collects + service_payments + ota_payouts
-- KHÔNG XÓA BẢNG CŨ
-- =============================================
...
-- 3) OTA Payouts Received (OTA_COLLECT - chỉ lấy status RECEIVED)
SELECT 
  op.id AS unified_payment_id,
  NULL::TEXT AS unified_booking_id, -- OTA payout là batch, không gắn 1 booking
  'OTA_COLLECT'::TEXT AS payment_channel,
  'OTA'::TEXT AS payer_type,
  'ROOMRISE'::TEXT AS payee_type, -- RULE: OTA luôn trả về Roomrise
...
'Unified view gom tất cả payments từ hotel_collects, service_payments, ota_payouts. 
Dùng cho dashboard và cashflow reporting. Không thay thế bảng gốc.';
