/**
 * channex-ari-webhook — "Always-200" queue-based receiver for ARI events
 * ──────────────────────────────────────────────────────────────────────
 * Design contract:
...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    return new Response(
      JSON.stringify({ ok: true, kind: "ari", ts: receivedAt }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  }
  ...
  try {
    ...
    if (payload && typeof payload === "object" && !Array.isArray(payload)) {
      ...
      eventType = (p.event as string) || (p.event_type as string) || "ari";
      propertyId = (p.property_id as string) || null;

      // Build dedupe from property + timestamp bucket (ARI events don't have unique IDs)
      if (propertyId) {
        const bucket = Math.floor(Date.now() / 5000); // 5-second bucket
        dedupeKey = `channex:ari:${propertyId}:${bucket}`;
      }
    }
