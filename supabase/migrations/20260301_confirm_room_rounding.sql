...
-- ============================================================
-- Allows marking a booking as "paid" when outstanding room
-- payment is within rounding tolerance (≤ 1,000 VND).
--
-- Inserts a balancing hotel_collects record so that
-- roomRemaining naturally becomes 0 at query time.
--
-- Does NOT modify booking revenue or expected amounts.
-- Does NOT create cashflow_entries (this is a write-off, not real cash).
-- ============================================================
...
DECLARE
  v_user_id        UUID;
  v_tolerance      INTEGER := 1000;  -- Must match PAYMENT_TOLERANCE_VND on FE
...
BEGIN
  ...
  -- ── GUARD 3: Idempotency — check if already applied ──
  ...
  IF v_existing_id IS NOT NULL THEN
    -- Already applied — return existing ID (idempotent)
  ...
  -- ── GUARD 4: Compute actual room remaining (same filter as FE) ──
  -- Room expected: from booking (total_amount_net)
  ...
  -- Room collected: sum of hotel_collects where related_type = 'ROOM' and not voided
  ...
  -- Verify the amount matches actual remaining (within ±1 for float rounding)
  ...
  -- ── ACTION: Insert hotel_collects record ──
  INSERT INTO hotel_collects (
    ...
    p_booking_id,
    v_room_remaining,         -- Use computed remaining, not p_amount (safer)
    'OTHER',                  -- Reuse existing safe value
    'ROOMRISE',               -- Roomrise absorbs the rounding
    'SYSTEM',                 -- Not a real person paying
    'ROOM',                   -- Matches FE filter
    'ROOM',                   -- collection type
    'ROUNDING_WRITE_OFF',     -- Identifier for idempotency + audit
    'COLLECTED',              -- Matches existing default
    now(),
    v_user_id
  )
