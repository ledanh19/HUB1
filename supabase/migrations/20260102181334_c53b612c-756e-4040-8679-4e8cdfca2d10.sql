...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'vn_provinces_read_all' AND tablename = 'vn_provinces') THEN
    CREATE POLICY "vn_provinces_read_all" ON public.vn_provinces FOR SELECT TO authenticated USING (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'vn_districts_read_all' AND tablename = 'vn_districts') THEN
    CREATE POLICY "vn_districts_read_all" ON public.vn_districts FOR SELECT TO authenticated USING (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'vn_wards_read_all' AND tablename = 'vn_wards') THEN
    CREATE POLICY "vn_wards_read_all" ON public.vn_wards FOR SELECT TO authenticated USING (true);
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'property_catalog' AND column_name = 'province_code') THEN
    ALTER TABLE public.property_catalog ADD COLUMN province_code TEXT REFERENCES public.vn_provinces(code);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'property_catalog' AND column_name = 'district_code') THEN
    ALTER TABLE public.property_catalog ADD COLUMN district_code TEXT REFERENCES public.vn_districts(code);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'property_catalog' AND column_name = 'ward_code') THEN
    ALTER TABLE public.property_catalog ADD COLUMN ward_code TEXT REFERENCES public.vn_wards(code);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'property_catalog' AND column_name = 'province_name_snapshot') THEN
    ALTER TABLE public.property_catalog ADD COLUMN province_name_snapshot TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'property_catalog' AND column_name = 'district_name_snapshot') THEN
    ALTER TABLE public.property_catalog ADD COLUMN district_name_snapshot TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'property_catalog' AND column_name = 'ward_name_snapshot') THEN
    ALTER TABLE public.property_catalog ADD COLUMN ward_name_snapshot TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'property_catalog' AND column_name = 'full_address') THEN
    ALTER TABLE public.property_catalog ADD COLUMN full_address TEXT;
  END IF;
END $$;
