...
-- Supports: ota_property_id, ota_source, channex_property_id filters.
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.rpc_get_ota_ar_dashboard_v2(
  p_property_id TEXT DEFAULT NULL,           -- ota_property_id
  p_source TEXT DEFAULT NULL,                -- ota_source
  p_channex_property_id TEXT DEFAULT NULL     -- channex_property_id (Chỗ nghỉ)
)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
...
DECLARE
  v_result JSONB;
  v_group_id CONSTANT TEXT := '72e58e1b-1e34-4678-9100-71c778ecf6d0'; -- AN_GIA
BEGIN
  WITH
  ...
  -- 7. Pending payouts — filtered by property/source through payout details
  pending_payouts AS (
    SELECT
  ...
  -- 11. Breakdown by ota_property_id + ota_source (includes OTA source per property)
  by_property AS (
    SELECT jsonb_agg(
      jsonb_build_object(
...
-- Grant execute to authenticated users (new 3-param signature)
GRANT EXECUTE ON FUNCTION public.rpc_get_ota_ar_dashboard_v2(TEXT, TEXT, TEXT)
  TO authenticated;
...
-- ============================================================================
-- Additive indexes (IF NOT EXISTS — safe to re-run)
-- ============================================================================
