-- PATCH: Add ota_source to by_property + filter pending_payouts by property/source
...
BEGIN
  ...
  ota_bookings AS (
    SELECT
      bm.unified_booking_id, bm.ota_source, bm.channex_property_id,
      bm.ota_property_id, bm.total_amount_net, bm.check_out_date
  ...
  eligible AS (
    SELECT
      ob.unified_booking_id, ob.ota_source, ob.channex_property_id,
      ob.ota_property_id, COALESCE(ob.total_amount_net, 0)::numeric AS amount,
      co.checkout_date, (CURRENT_DATE - co.checkout_date) AS days_since_checkout
    ...
    WHERE wp.unified_booking_id IS NULL AND wd.unified_booking_id IS NULL
  ),
  pending_payouts AS (
    SELECT
  ...
  summary AS (
    SELECT COALESCE(SUM(amount), 0) AS eligible_amount, COUNT(*) AS eligible_count
    FROM eligible
  ),
  aging AS (
  ...
  by_source AS (
    SELECT jsonb_agg(
      jsonb_build_object('source', ota_source, 'amount', src_amount, 'count', src_count)
  ...
  by_property AS (
    ...
    FROM (
      SELECT
        ...
        SUM(e.amount) AS prop_amount, COUNT(*) AS prop_count
      FROM eligible e
      GROUP BY COALESCE(e.ota_property_id, 'unknown'), e.ota_source
    ) prop
    LEFT JOIN channex_user_properties cup ON cup.channex_property_id = prop.channex_prop_id
  )
  SELECT jsonb_build_object(
    'summary', jsonb_build_object(
      'eligible_amount', s.eligible_amount, 'eligible_count', s.eligible_count,
      'pending_amount', pp.pending_amount, 'pending_count', pp.pending_count,
      'total_outstanding', s.eligible_amount + pp.pending_amount,
      'overdue_payout_count', pp.overdue_count, 'overdue_payout_amount', pp.overdue_amount
    ),
    'aging', a.buckets,
...
GRANT EXECUTE ON FUNCTION public.rpc_get_ota_ar_dashboard_v2(TEXT, TEXT) TO authenticated;
