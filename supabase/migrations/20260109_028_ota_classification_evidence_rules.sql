...
-- ============================================================
-- Date: 2026-01-09
-- Purpose: Fix Sprint 1 issues before merge
--   1. Add require_evidence column with classification-based defaults
--   2. Update DONE guard to respect require_evidence flag
--   3. Update trigger for field-level immutability (not record-level)
--   4. Backfill existing tasks based on classification
-- 
-- Rules:
--   EXECUTION: require_evidence = true, min_evidence_count >= 1
--   PREP/AUTO: require_evidence = false, min_evidence_count = 0
--   OPS: require_evidence = false (can be enabled manually)
--
-- Rollback:
--   ALTER TABLE ota_tasks DROP COLUMN IF EXISTS require_evidence;
-- ============================================================
...
-- EXECUTION → require_evidence = true, min_evidence_count = 1
-- PREP/AUTO → require_evidence = false, min_evidence_count = 0
-- OPS → require_evidence = false, min_evidence_count = 0
-- ============================================================
...
SET 
  require_evidence = CASE 
    ...
    ELSE true  -- default for NULL classification
  END,
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.classification IS DISTINCT FROM NEW.classification) THEN
    ...
    ELSE
      -- NULL classification defaults to EXECUTION behavior
      NEW.require_evidence := COALESCE(NEW.require_evidence, true);
...
BEGIN
  ...
  -- ============================================================
  -- DONE GUARD: Check evidence ONLY if require_evidence = true
  -- ============================================================
  IF UPPER(p_new_status) = 'DONE' THEN
    ...
    -- Only enforce evidence check if required
    IF v_require_evidence AND v_min_required > 0 THEN
      ...
      END IF;
    END IF;
    -- If require_evidence = false, skip evidence check (PREP/AUTO/OPS)
  END IF;
...
BEGIN
  ...
  IF NEW.status = 'DONE' AND (OLD.status IS NULL OR OLD.status != 'DONE') THEN
    ...
    -- Only enforce if require_evidence = true
    IF v_require_evidence AND v_min_required > 0 THEN
      SELECT COUNT(*) INTO v_approved_count
      FROM ota_task_evidence
      ...
      END IF;
    END IF;
    -- If require_evidence = false (PREP/AUTO/OPS), allow DONE without evidence
  END IF;
...
-- Allow updates to metadata fields even on DONE tasks
-- ============================================================
...
DECLARE
  ...
  v_field TEXT;
BEGIN
  -- Only check if task is DONE and not being un-done by admin override
  IF OLD.status = 'DONE' THEN
    -- Check each immutable field
    FOREACH v_field IN ARRAY v_immutable_fields
    LOOP
      -- Use dynamic check for field changes
    ...
    -- These metadata fields CAN be updated on DONE tasks (with audit)
  END IF;
...
-- ============================================================
-- Fix: Ensure RPC checks project access for EACH task/evidence
-- and only approves PENDING or NEEDS_REVISION (not APPROVED/REJECTED)
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    IF NOT FOUND THEN
      -- Task doesn't exist, skip with audit note
    ...
    -- FIX 3: CRITICAL - Check project access per task
    IF NOT has_ota_project_access(v_task.project_id) THEN
      -- No access to this project, skip with audit note
      v_skipped_count := v_skipped_count + 1;
      v_results := array_append(v_results, json_build_object(
        'task_id', v_task_id,
        'status', 'SKIPPED',
    ...
    -- Approve ONLY PENDING or NEEDS_REVISION evidence (NOT APPROVED/REJECTED)
    FOR v_evidence IN
      SELECT id, task_id, review_status
      FROM ota_task_evidence
      WHERE task_id = v_task_id
      AND review_status IN ('PENDING', 'NEEDS_REVISION')  -- FIX: Only approvable statuses
    LOOP
      ...
      -- Log individual evidence approval to audit
    ...
    -- Record per-task result with details
    v_results := array_append(v_results, json_build_object(
      'task_id', v_task_id,
      'project_id', v_task.project_id,
      'status', 'PROCESSED',
  ...
  -- Insert bulk action summary audit log entry
  INSERT INTO ota_audit_log (
    action,
    entity_type,
    entity_id,
...
RAISE NOTICE 'Migration 028_ota_classification_evidence_rules completed successfully';
