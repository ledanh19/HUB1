-- =============================================
-- ADD payment_link_url TO hotel_collects
-- Sprint: Payment Link URL Enhancement
-- Date: 2026-03-03
-- 
-- PURPOSE: Lưu trữ URL link thanh toán khi CSKH ghi nhận thu tiền
-- qua phương thức PAYMENT_LINK. Giúp truy vết giao dịch và 
-- hỗ trợ đối soát.
--
-- ADDITIVE ONLY - Non-breaking change
-- =============================================

-- STEP 1: Add payment_link_url column
DO $$
BEGIN
  IF NOT EXISTS (
...
-- STEP 2: Add payment_provider column (nếu chưa có - lưu provider như ONEPAY, NINEPAY, etc.)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
