...
-- Created: 2026-02-20 | Revised: 2026-02-21
-- NON-BREAKING + ADDITIVE: new tables only, no existing changes
--
-- ARCHITECTURE: Shared workspace with RBAC access control.
-- Email is an internal team tool, NOT personal inbox per user.
-- Access governed by has_role() / has_email_access() functions.
-- Roles: admin/super_admin = full, cskh = view+reply, sale = view
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.email_accounts (
  ...
  -- ⚠️  Encrypted with AES-256-GCM; NEVER plain text
  token_cipher   text,
  refresh_cipher text,
  ...
  syncing_since  timestamptz,     -- concurrency lock: set when sync starts, null when done
  -- Error tracking
  error_code     text,
  error_at       timestamptz,
  -- Visibility: TEAM = shared inbox, PRIVATE = creator-only
...
CREATE TABLE IF NOT EXISTS public.email_messages_mirror (
  ...
  date                 timestamptz,
  subject              text,
  headers              jsonb DEFAULT '{}'::jsonb,   -- Message-ID, In-Reply-To, References
  body_plain           text,
...
-- ─── 5. email_oauth_states (CSRF-safe, DB-persisted) ────────
...
-- ═══════════════════════════════════════════════════════════
-- 7a. DROP old tenant_id = auth.uid() policies (v2 → v3)
-- Safe: IF EXISTS = no-op on fresh deploy.
-- ═══════════════════════════════════════════════════════════
...
-- Also drop new-name policies idempotently (re-run safe)
...
-- ═══════════════════════════════════════════════════════════
-- 7b. RBAC SHARED WORKSPACE MODEL
-- ─────────────────────────────────────────────────────────
-- Email is accessible to ALL authorized roles, not per-user.
-- Permission matrix:
--   admin / super_admin  → view + reply + manage accounts
--   cskh                 → view + reply
--   sale                 → view only
--   ke_toan / others     → no access (403)
--
-- Helper: has_email_access() → TRUE if current user has any
-- email-view role. Used in SELECT policies across all tables.
-- ═══════════════════════════════════════════════════════════

-- 7c. Create helper BEFORE policies that reference it
...
-- ── email_oauth_states: only service_role should touch this ─
-- No authenticated user policy = cannot read/write from client.
GRANT ALL ON public.email_oauth_states TO service_role;
REVOKE ALL ON public.email_oauth_states FROM anon, authenticated;

-- ─── 8. Column-level security: DENY token columns to anon/authenticated ─
-- Only the service_role (backend) can SELECT token_cipher/refresh_cipher.
-- Revoke default, then grant everything except token columns.
...
-- Full access for service_role (backend only)
GRANT ALL ON public.email_accounts TO service_role;

-- Threads + Messages: SELECT only for authenticated (writes via service_role)
